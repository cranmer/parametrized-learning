...
%The point here is to make the distinction of $f(x|\theta)$ in this context from a generic generative classifier like normal discriminate analysis or naive Bayes. 
...
Since the prior $p(y)$ is not needed for the target likelihood ratio test and because the classifier score $p(y|x)$ may not be well calibrated, we choose to denote the classifier score $s(x)$ and simply think of it as a deterministic dimensionality reduction map $s: X \to \mathbb{R}$. 
...
%
%\begin{equation}
%\frac{f(x|\theta_0)}{f(x|\theta_1)}= \sum_c \frac{ w_c(\theta_0) f_c(x| \theta_0)}{\sum_{c'} w_{c'}(\theta_1) f_{c'}(x| \theta_1)} 
%\end{equation}
...
\frac{f(x|\theta_0)}{f(x|\theta_1)} & =& \frac{\sum_c w_c(\theta_0) f_c(x| \theta_0)}{\sum_c w_c(\theta_1) f_c(x| \theta_1)} 
\\
%&=& \sum_c \frac{ w_c(\theta_0) f_c(x| \theta_0)}{\sum_{c'} w_{c'}(\theta_1) f_{c'}(x| \theta_1)} \\
%&=& \sum_c \left[ \frac{\sum_{c'} w_{c'}(\theta_1) f_{c'}(x| \theta_1)}{ w_c(\theta_0) f_c(x| \theta_0)}  \right]^{-1} \\
